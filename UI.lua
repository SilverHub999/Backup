...
local ClosureBindings = {
	...
	[12] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(12)local Root = script.Parent.Parent
		...
		function Notification:New(Config)
			...
			for Index, ButtonConfig in next, Config.Buttons do
				local ButtonTitle = ButtonConfig.Title or "Button"
				local ButtonCallback = ButtonConfig.Callback or function() end
				local NumButtons = #Config.Buttons

				local Button = require(Root.Components.Button)("", NewNotification.ButtonHolder, true)
				Button.Title.Text = ButtonTitle
				Button.Frame.Size = UDim2.new(1 / NumButtons, -(((NumButtons - 1) * 8) / NumButtons), 0, 30)

				Creator.AddSignal(Button.Frame.MouseButton1Click, function()
					require(Root):SafeCallback(ButtonCallback)
					NewNotification:Close()
				end)

				NewNotification.Buttons[Index] = Button
			end
